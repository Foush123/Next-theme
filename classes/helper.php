...
class helper {
    ...
    public function load_additional_scss_settings() {
        $scss = '';
        $primary = theme_next_theme_get_setting('primarycolor');
        $secondary = theme_next_theme_get_setting('secondarycolor');
        $slideoverlayval = theme_next_theme_get_setting('slideOverlay');
        $slideopacity = (!empty($slideoverlayval)) ? $this->get_hexa('#000000', $slideoverlayval) : 0.4;
        $footerbgoverlayval = theme_next_theme_get_setting('footerbgOverlay');

        if (empty($primary)) {
            switch (theme_next_theme_get_setting('preset')) {
                case 'eguru':
                    $primary = '#a55ba5';
                    break;
                case 'klass':
        ...
        $pagesizecustomval = theme_next_theme_get_setting('pagesizecustomval');
        $fontsize = theme_next_theme_get_setting('fontsize');
    ...
    public function render_slideimg($p, $slidername) {
        ...
        $slideimage = '';
        if (theme_next_theme_get_setting($slidername)) {
