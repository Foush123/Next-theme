...
class next_theme_blocks {
    ...
    public function sitefeatures() {
        global $OUTPUT, $PAGE;
        $status = theme_next_theme_get_setting('sitefblockstatus');
        $blocktitle = theme_next_theme_lang(theme_next_theme_get_setting('sitefeaturetitle'));
        $blockdesc = theme_next_theme_lang(theme_next_theme_get_setting('sitefeaturedesc'));
        $items = [];
        (int) $cs = 0;
        if ($status == 1) {
            $features = theme_next_theme_get_setting('numberofsitefeature');
            $block['class'] = 'icon-block';
            for ($i = 1; $i <= $features; $i++) {
                $sfbstatus = theme_next_theme_get_setting('sitefblock'.$i.'status');
                $sfbicon = theme_next_theme_get_setting('sitefblock'.$i.'icon');
                $sfbtitle = theme_next_theme_get_setting('sitefblock'.$i.'title');
                $sfbcontent = theme_next_theme_get_setting('sitefblock'.$i.'content');
            ...
            for ($i = 1; $i <= $features; $i++) {
                $sfbtitle = theme_next_theme_get_setting('sitefblock'.$i.'title');
                $sfbtitle = theme_next_theme_lang($sfbtitle);
                $sfbcontent = trim(theme_next_theme_get_setting('sitefblock'.$i.'content'));
                $sfbcontent = theme_next_theme_lang($sfbcontent);
                $sfbstatus = theme_next_theme_get_setting('sitefblock'.$i.'status');
                $sfbicon = theme_next_theme_get_setting('sitefblock'.$i.'icon');
                $sfbicon = theme_next_theme_lang($sfbicon);
                $sfbbody = (!empty($sfbtitle) || (!empty($sfbcontent)) || (!empty($sfbicon))) ? true : false;
                $sfurl = theme_next_theme_get_setting('sitefblock'.$i.'url');
            ...
            return $OUTPUT->render_from_template('theme_next_theme/next_theme_blocks', $block);
        }
    }
    ...
    public function marketingspot() {
        global $OUTPUT, $PAGE;
        $status = theme_next_theme_get_setting('mspotstatus');
        if ($status == 1) {
            $mspot['title'] = theme_next_theme_lang(theme_next_theme_get_setting('mspottitle'));
            $mspot['desc'] = theme_next_theme_lang(theme_next_theme_get_setting('mspotdesc'));
            $mspot['content'] = theme_next_theme_get_setting('mspotcontent', 'format_html');
            $mspot['media'] = theme_next_theme_get_setting('mspotmedia', 'file');
            ...
            return $OUTPUT->render_from_template('theme_next_theme/next_theme_blocks', $mspot);
        }
    }
    ...
    public function jumbotron() {
        global $OUTPUT, $PAGE;
        $status = theme_next_theme_get_setting('jumbotronstatus');
        if ($status == 1 ) {
            $jumbotron['title'] = theme_next_theme_lang(theme_next_theme_get_setting('jumbotrontitle'));
            $jumbotron['desc'] = theme_next_theme_lang(theme_next_theme_get_setting('jumbotrondesc'));
            $jumbotron['btntext'] = theme_next_theme_lang(theme_next_theme_get_setting('jumbotronbtntext'));
            $jumbotron['buttonlink'] = theme_next_theme_get_setting('jumbotronbtnlink');
            $btntarget = theme_next_theme_get_setting('jumbotronbtntarget');
            ...
            return $OUTPUT->render_from_template('theme_next_theme/next_theme_blocks', $jumbotron);
        }
    }
}
